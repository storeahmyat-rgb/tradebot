...
from .strategy import EMAQuadStrategy, Signal
...
class SymbolWorker(threading.Thread):
    ...
    def _tick(self):
        ...
        if result.just_crossed and signal in (Signal.BUY, Signal.SELL):
            self.engine._emit("log", {
                "level": "info",
        ...
        if signal == Signal.BUY:
            ...
            self.engine._emit("log", {
                ...
                "msg": (f"[{symbol}] 🟢 FRESH BUY cross -> opening LONG qty={trade_size:.6f} @ ~{mark_price:.4f} | "
                        f"SL={sl_pct}% TP={tp_pct}% (STRICT 1:3 RR) | "
                        f"EMA8={e8:.4f} EMA13={e13:.4f} EMA21={e21:.4f} EMA55={e55:.4f}")
            })
            ...
            if order.get("success"):
                ...
                # Place SL and TP on Binance IMMEDIATELY (no delay)
...
class BotEngine:
    ...
    def force_stop(self):
        ...
        self._emit("log", {
            "level": "warn",
            "msg": "⚠️ FORCE STOP done. All state reset. You can START fresh now."
        })
        return {"success": True, "message": "Force stop done - all state reset"}
